...
namespace CertificateManagementApi.Service.Implementation
{
    public class OrganizationService : IOrganizationService
    {
        ...
        public async Task<BaseResponse<OrganizationDto>> Approve(int id)
        {
        ...
        public async Task<BaseResponse<OrganizationDto>> Create(CreateOrganizationRequestModel model)
        {
            ...
            if (model.CAC != null)
            {
                ...
                var imagePath = Path.Combine(imgPath, "CAC");
                Directory.CreateDirectory(imagePath);
                var imageType = model.CAC.ContentType.Split('/')[1];
                CACImage = $"{Guid.NewGuid()}.{imageType}";
                var fullPath = Path.Combine(imagePath, CACImage);
                using (var fileStream = new FileStream(fullPath, FileMode.Create))
                {
                    model.CAC.CopyTo(fileStream);
                }
            }
            ...
            {
                var imgPath = _webHostEnvironment.WebRootPath;
                var imagePath = Path.Combine(imgPath, "Logo");
                Directory.CreateDirectory(imagePath);
                var imageType = model.CAC.ContentType.Split('/')[1];
                logoImage = $"{Guid.NewGuid()}.{imageType}";
                var fullPath = Path.Combine(imagePath, logoImage);
                using (var fileStream = new FileStream(fullPath, FileMode.Create))
                {
        ...
        public async Task<OrganizationsResponseModels> GetAllUnApprovedOrganization()
        {
            var organizations = await _organizationRepository.GetUnapprovedOrganizations();
            if (organizations == null) return new OrganizationsResponseModels
            {
                Message = "Organization is null",
                Status = false,
